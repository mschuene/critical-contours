#!/bin/bash
cd "$(dirname "$0")"
mkdir best_threshold_k100
cd best_threshold_k100
jobid=$(qsub -terse -N best_threshold_k100 -q `cat /0/maik/queue_maximus.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m a `#end abort suspend` -t 1 <<EOT
...
outdir = "../avalanches/best_threshold_k100"
try:
    os.mkdir(outdir)
except:
    pass
...
qsub -N postprocessbest_threshold_k100 -q `cat /0/queue_maximus.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m ea `#end abort suspend` -l h_rt=00:00:15 -hold_jid $jid <<EOT
source $HOME/.bashrc
python <<EOF
EOF
EOT
