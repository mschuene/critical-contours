#!/bin/bash
cd "$(dirname "$0")"
mkdir simple_submatrix
cd simple_submatrix
jobid=$(qsub -terse -N simple_submatrix -q `cat /0/maik/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m a `#end abort suspend` -t 1-100 <<EOT
...
outdir = "../avalanches/simple_submatrix"
try:
    os.mkdir(outdir)
except:
    pass
...
activations = pickle.load(open('activated_ensembles.pickle','rb'))
...
qsub -N postprocesssimple_submatrix -q `cat /0/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m ea `#end abort suspend` -l h_rt=00:00:15 -hold_jid $jid <<EOT
source $HOME/.bashrc
python <<EOF
EOF
EOT
