#!/bin/bash
cd "$(dirname "$0")"
mkdir 2_afc_parameter_search
cd 2_afc_parameter_search
jobid=$(qsub -terse -N 2_afc_parameter_search -q `cat /0/maik/queue_maximus+long.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m a `#end abort suspend` -t 1-160 <<EOT
...
outdir = "../avalanches/2_afc_parameter_search"
try:
    os.mkdir(outdir)
except:
    pass
...
qsub -N postprocess2_afc_parameter_search -q maximus+long -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m ea `#end abort suspend` -l h_rt=00:00:15 -hold_jid $jid <<EOT
source $HOME/.bashrc
python <<EOF
EOF
EOT
