#!/bin/bash
cd "$(dirname "$0")"
mkdir evaluate_sim_2afc_population_activity_k300
cd evaluate_sim_2afc_population_activity_k300
jobid=$(qsub -terse -N evaluate_sim_2afc_population_activity_k300 -q `cat /0/maik/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m a `#end abort suspend` -t 1-160 <<EOT
...
outdir = "../avalanches/evaluate_sim_2afc_population_activity_k300"
try:
    os.mkdir(outdir)
except:
    pass
...
qsub -N postprocessevaluate_sim_2afc_population_activity_k300 -q `cat /0/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m ea `#end abort suspend` -l h_rt=00:00:15 -hold_jid $jid <<EOT
source $HOME/.bashrc
python <<EOF
EOF
EOT
