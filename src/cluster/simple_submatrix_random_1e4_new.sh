#!/bin/bash
cd "$(dirname "$0")"
mkdir simple_submatrix_random_1e4_new
cd simple_submatrix_random_1e4_new
jobid=$(qsub -terse -N simple_submatrix_random_1e4_new -q `cat /0/maik/queue_long.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m a `#end abort suspend` -pe omp 2 -t 1-100 <<EOT
...
outdir = "../avalanches/simple_submatrix_random_1e4_new"
try:
    os.mkdir(outdir)
except:
    pass
...
qsub -N postprocesssimple_submatrix_random_1e4_new -q `cat /0/queue_long.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m ea `#end abort suspend` -l h_rt=00:00:15 -hold_jid $jid <<EOT
source $HOME/.bashrc
python <<EOF
EOF
EOT
