#!/bin/bash
cd "$(dirname "$0")"
mkdir const_crit_subnetwork1e31e4
cd const_crit_subnetwork1e31e4
jobid=$(qsub -terse -N const_crit_subnetwork1e31e4 -q `cat /0/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m a `#end abort suspend` -t 1-10 <<EOT
...
outdir = "../avalanches/const_crit_subnetwork1e31e4"
try:
    os.mkdir(outdir)
except:
    pass
...
qsub -N postprocessconst_crit_subnetwork1e31e4 -q `cat /0/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m ea `#end abort suspend` -l h_rt=00:00:15 -hold_jid $jid <<EOT
source $HOME/.bashrc
python <<EOF
EOF
EOT
