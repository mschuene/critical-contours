#!/bin/bash
cd "$(dirname "$0")"
mkdir simple_submatrix_random
cd simple_submatrix_random
jobid=$(qsub -terse -N simple_submatrix_random -q `cat /0/maik/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m a `#end abort suspend` -t 1-100 <<EOT
...
outdir = "../avalanches/simple_submatrix_random"
try:
    os.mkdir(outdir)
except:
    pass
...
qsub -N postprocesssimple_submatrix_random -q `cat /0/queue_long_64gb.txt` -cwd `#execute in current directory` -p 0 -M maikschuenemann@gmail.com -m ea `#end abort suspend` -l h_rt=00:00:15 -hold_jid $jid <<EOT
source $HOME/.bashrc
python <<EOF
EOF
EOT
